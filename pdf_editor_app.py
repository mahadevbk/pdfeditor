...
def pdf_to_images(uploaded_file):
    ...
    tmp.close()
    imgs = convert_from_path(tmp.name)
    os.unlink(tmp.name)
    outs = []
...
def pdf_to_docx(uploaded_file):
    ...
    os.unlink(tmp.name)
    for im in imgs:
        doc.add_paragraph(pytesseract.image_to_string(im))
    buf = io.BytesIO()
    doc.save(buf)
    buf.seek(0)
    return buf
...
if not op:
    ...
    if op == "Convert Ebook":
        ...
        if st.button("Convert") and f:
            try:
                converted_data = convert_ebook(f, out_format)
                ...
                st.error(f"❌ Conversion function limited due to Streamlit cloud limitations on Root access, The code works but not on streamlit clould. Download the repository folder from Github and run on your local machine. Use pip -r requirements to install the script requirements.: {e}")
        ...
        if st.button("Extract Metadata") and f:
            ...
            st.success("✅ Metadata!")
            st.download_button("Download", data=out, file_name='metadata.txt')
    # Advanced
    elif op == "Encrypt PDF":
        ...
        if st.button("Delete Pages") and f and rng:
            try:
                ...
                out = delete_pages(f, pages)
                st.success("✅ Pages deleted!")
                st.download_button("Download", data=out, file_name='deleted.pdf')
            except:
